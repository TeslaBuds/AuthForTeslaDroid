...
namespace TeslaAuth
{
    public class TeslaAuthHelper
    {
        ...
        public delegate Task<string> GetAuthorizationCodeAsyncDelegate(string codeUrl, CancellationToken cancellationToken);

        public async Task<Tokens> AuthenticateAsync(GetAuthorizationCodeAsyncDelegate getAuthorizationCodeAsync, TeslaAccountRegion region = TeslaAccountRegion.Unknown, CancellationToken cancellationToken = default)
        {
            return await AuthenticateAsync("", "", null, region, cancellationToken, getAuthorizationCodeAsync);
        }

        public async Task<Tokens> AuthenticateAsync(string username, string password, string mfaCode = null, TeslaAccountRegion region = TeslaAccountRegion.Unknown, CancellationToken cancellationToken = default, GetAuthorizationCodeAsyncDelegate getAuthorizationCodeAsync = null)
        {
            
            var client = clients.GetOrAdd(region, CreateHttpClient);

            var loginInfo = await InitializeLoginAsync(client, cancellationToken);
            var code = getAuthorizationCodeAsync != null ? await getAuthorizationCodeAsync(GetAuthorizationCodeUrl(loginInfo, client), cancellationToken) : await GetAuthorizationCodeAsync(username, password, mfaCode, loginInfo, client, cancellationToken);
        ...
        async Task<string> GetAuthorizationCodeAsync(string username, string password, string mfaCode, LoginInfo loginInfo, HttpClient client, CancellationToken cancellationToken)
        {
            ...
            string url = GetAuthorizationCodeUrl(loginInfo, client);
        ...
        private static string GetAuthorizationCodeUrl(LoginInfo loginInfo, HttpClient client)
        {
            var b = new UriBuilder(client.BaseAddress + "oauth2/v3/authorize") { Port = -1 };
            var q = HttpUtility.ParseQueryString(b.Query);
            q["client_id"] = "ownerapi";
